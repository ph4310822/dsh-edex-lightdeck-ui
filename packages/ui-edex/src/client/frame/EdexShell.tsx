...
import type { CommandResult } from '@danielng23/dsh-lightdeck-host-system-metrics/types'
...
/** The workspace's own title strip height (CSS mirror: --edex-center-title-h). */
export const CENTER_TITLE_HEIGHT = '22px'
 ...
 * vocabulary. The section's own treatment is neutralized in CSS (the
 * workspace must stay transparent — it is the card's content surface); the
 * visible chrome (title strip + hairline border) lives on the container.
 */
...
export function EdexShell({
  ...
  useLayoutEffect(() => {
    ...
    // original UI tiles from the viewport top like the shell bars. The
    // center chrome (card gap + title strip + card border) must NOT cover
    // the workspace: the frame insets below the strip by exactly that much.
    const shellStyle = getComputedStyle(shell)
    const gap = (shellStyle.getPropertyValue('--edex-card-gap') || '0px').trim()
    frame.style.top = `calc(${TOP_BAR_HEIGHT} + ${gap} + ${CENTER_TITLE_HEIGHT} + 1px)`
    frame.style.left = `calc(${CENTER_LEFT} + ${gap})`
    frame.style.right = `calc(${CENTER_RIGHT} + ${gap})`
    frame.style.bottom = `calc(${CENTER_BOTTOM} + ${gap})`
  ...
  return (
    ...
    >
      ...
      {/* The ORIGINAL UI framed as the reference's card: the container
          (.centerWidget) carries the hairline border + the white title strip
          (same header language as every card), and the reshaped workspace
          insets below the strip so nothing is covered. The WidgetSection
          itself stays transparent — the workspace IS the card body. */}
      <section className={css.centerWidget}>
        <div className={css.centerTitle} data-testid="edex-center-title">DSH WORKSPACE</div>
